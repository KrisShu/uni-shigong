...
export function uploadOne(
    ...
    const {
        url = 'http://192.168.110.208:8655/app/emp/common/fileUpload',
        fieldName = 'file',
        headers = {},
        formData = {},
        ...
        const task = uni.uploadFile({
            ...
            success: res => {
                try {
                    ...
                    if (data?.code === 2000) {
                        const raw = data.data || data.url || data.path;
                        const finalUrl = String(raw || '').startsWith('http') ? raw : imageBase + raw;
