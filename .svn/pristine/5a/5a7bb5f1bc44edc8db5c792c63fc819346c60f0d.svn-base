...
function needAuthorize(info: any): boolean {
     ...
     */
    // 未登录 / 过期 / 被顶
    if (code === 1001 || code === 1002 || code === 1003 || code === 1009 || code === 1010) {
...
export function http<TResp = any, TData = any>(opts: HttpOptions<TData>) {
    const GlobalStore = useGlobalStore();
    if (GlobalStore.token != uni.getStorageSync('token')) {
        tip('登录状态失效，请重新登录');
        GlobalStore.loginOut();
        return;
    }
    ...
    if (encrypt && (method === 'POST' || method === 'PUT')) {
        ...
        payload = {
            ...
            // clearText: import.meta.env.NODE_ENV === 'development' ? data : '', // ⚠️ 生产不要传明文
