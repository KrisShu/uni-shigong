            ...
            <view v-else>
                <view @click="goToDetail(item)" v-for="item in projectList" :key="item.id" class="project-card">
                    <view class="project-id">
                        {{ item.batchno }}
                        <text class="special-id" v-show="item.demandBatchno">{{ item.demandBatchno }}</text>
                    </view>
...
<script lang="ts" setup>
    ...
    const fetchProjects = async (reset = false) => {
        ...
        try {
            const res = await API.CUS_ProjectList({
                name: keyword.value,
    ...
    const switchTab = (index: number) => {
        filterStore.set({ empIndexActiveTab: index, scrollTop: 0, cusPageNo: 1 }); // 记录回到顶部
    ...
    async function restoreAndFetch(reset: boolean) {
        ...
        // 先恢复筛选（你的逻辑）
        activeTab.value = Number(filterStore.empIndexActiveTab) ?? 0;
        keyword.value = filterStore.empIndexKeyword ?? '';
        page.value = filterStore.cusPageNo ?? 1;
        const pageName = filterStore.pageName || '';

        // 再拉列表（会触发 DOM 生成）
        if (reset || pageName === 'cusMine') {
            await fetchProjects(true);
        }
    ...
    onShow(() => {
        ...
        // 来自个人中心并需要恢复/刷新时触发
        if (filterStore.pageName === 'cusMine') {
