...
<template>
    <scroll-view
        class="pd-accept-scroll-area common-scroll-area"
        scroll-y
        ...
        <view v-else class="accept-template" v-for="(item, index) in acceptanceList" :key="index">
            <view class="accept-warp">
                <view class="flex j-between a-center">
                    ...
                    <text class="name">{{ item.empName }}</text>
                </view>

                <view class="status-text">
                    {{ getAcceptStatus(item) }}
                </view>
                <view class="status-desc flex" v-if="item.reason">
                    <text class="label">
                        {{ item.type == 1 ? $t('staff.abarbeitung.key_MS') : $t('staff.pro.check.key_YSMX') }}：
                    </text>
                    <text class="value">{{ item.reason }}</text>
                </view>

                <ImagePreview v-show="item.acceptancePath.length" :images="item.acceptancePath" />
            </view>
        </view>
    </scroll-view>
</template>
...
<script lang="ts" setup>
    ...
    const getAcceptanceList = async () => {
        ...
        try {
            const res = await API.EMP_ProjectAcceptanceList({
                id: props.projectId,
            });
